...
fun contentSource(name: String?): ContentSource {
	...
	// GlobalExtensionManager.mangaSources.value.find { it.name == name }?.let { return com.nyora.hasan72341.core.parser.nyora.NyoraParserSource(it) }
...
fun ContentSource.getSummary(context: Context, contentType: ContentType? = null): String? = when (val source = unwrap()) {
	is com.nyora.hasan72341.mihon.model.MihonMangaSource -> {
...
fun ContentSource.getOriginLabel(context: Context): String? = when (this) {
	...
	is com.nyora.hasan72341.mihon.model.MihonMangaSource -> "Mihon"
	else -> null
}
...
fun ContentSource.getTitle(context: Context): String {
	val baseTitle = when (val source = unwrap()) {
		...
		is com.nyora.hasan72341.mihon.model.MihonMangaSource -> source.displayName
		else -> {
...
val ContentSource.isBroken: Boolean
	get() {
		...
		return when (unwrapped) {
			is NyoraParserSource -> unwrapped.isBroken
			else -> {
				// com.nyora.hasan72341.core.extensions.GlobalExtensionManager.contentSources.value.find { it.originalSource == unwrapped || it.name == unwrapped.name }?.isBroken == true ||
				// com.nyora.hasan72341.core.extensions.GlobalExtensionManager.mangaSources.value.find { it.originalSource == unwrapped || it.name == unwrapped.name }?.isBroken == true
				false
			}
		}
	}
