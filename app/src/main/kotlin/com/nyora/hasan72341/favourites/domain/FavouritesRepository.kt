...
import com.nyora.hasan72341.favourites.data.toFavouriteCategory
import com.nyora.hasan72341.favourites.data.toMangaList
...
class FavouritesRepository @Inject constructor(
	...
	fun observeCategoriesIds(mangaId: String): Flow<Set<Long>> {
		return db.getFavouritesDao().observeIds(mangaId).map { it.toSet() }
	}

	fun observeCategories(mangaId: String): Flow<Set<FavouriteCategory>> {
	...
	suspend fun getCategoriesIds(mangaId: String): Set<Long> {
	...
	suspend fun removeFromFavourites(ids: Collection<String>): ReversibleHandle {
		db.withTransaction {
			for (id in ids) {
	...
	suspend fun removeFromCategory(categoryId: Long, ids: Collection<String>): ReversibleHandle {
		db.withTransaction {
			for (id in ids) {
	...
	private suspend fun recoverToFavourites(ids: Collection<String>) {
		db.withTransaction {
			for (id in ids) {
	...
	private suspend fun recoverToCategory(categoryId: Long, ids: Collection<String>) {
		db.withTransaction {
			for (id in ids) {
