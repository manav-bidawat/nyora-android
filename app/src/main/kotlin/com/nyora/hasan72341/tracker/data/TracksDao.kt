...
abstract class TracksDao : MangaQueryBuilder.ConditionCallback {
	...
	abstract suspend fun find(mangaId: String): TrackEntity?

	@Query("SELECT IFNULL(chapters_new,0) FROM tracks WHERE manga_id = :mangaId")
	abstract suspend fun findNewChapters(mangaId: String): Int
	...
	@Query("SELECT IFNULL(chapters_new, 0) FROM tracks WHERE manga_id = :mangaId")
	abstract fun observeNewChapters(mangaId: String): Flow<Int>
	...
	abstract suspend fun clearCounter(mangaId: String)

	@Query("DELETE FROM tracks WHERE manga_id = :mangaId")
	abstract suspend fun delete(mangaId: String)
