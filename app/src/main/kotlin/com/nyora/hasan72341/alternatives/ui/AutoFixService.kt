...
import com.nyora.hasan72341.alternatives.domain.AutoFixUseCase
import com.nyora.hasan72341.alternatives.domain.AutoFixUseCase.NoAlternativesException
		...
		for (mangaId in ids) {
			powerManager.withPartialWakeLock(TAG) {
				val result = runCatchingCancellable {
					autoFixUseCase.invoke(mangaId.toString())
				}
				...
				notification.setLargeIcon(
					coil.execute(
						ImageRequest.Builder(this)
							...
							.mangaSourceExtra(replacement.source.toMangaSource())
							.build(),
					).toBitmapOrNull(),
				)
				...
				notification
					...
					.setContentText(
						getString(
							...
							seed.title,
							seed.source.toMangaSource().getTitle(this),
							replacement.title,
							replacement.source.toMangaSource().getTitle(this),
						),
					)
