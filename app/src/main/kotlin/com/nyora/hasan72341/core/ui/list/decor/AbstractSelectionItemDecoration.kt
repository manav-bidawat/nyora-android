package com.nyora.hasan72341.core.ui.list.decor
...
abstract class AbstractSelectionItemDecoration : RecyclerView.ItemDecoration() {
	...
	protected val selection = mutableSetOf<String>()
	...
	val checkedItemsIds: Set<String>
		get() = selection
	...
	fun setItemIsChecked(id: String, isChecked: Boolean) {
		if (isChecked) {
			selection.add(id)
		} else {
			selection.remove(id)
	...
	fun checkAll(ids: Collection<String>) {
		for (id in ids) {
			selection.add(id)
		}
	}
	...
	private fun doDraw(canvas: Canvas, parent: RecyclerView, state: RecyclerView.State, isOver: Boolean) {
		...
		for (child in parent.children) {
			...
			if (itemId != null && itemId in selection) {
	...
	abstract fun getItemId(parent: RecyclerView, child: View): String?
