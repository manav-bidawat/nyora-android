...
class CloudFlareActivity : BaseBrowserActivity(), CloudFlareCallback {
	...
	private fun shouldUseInterception(source: MangaSource, repository: ParserMangaRepository?): Boolean {
		...
		// Check if parser has the InterceptCloudflare ConfigKey (matched by its key)
		...
		val interceptKey = configKeys.firstOrNull { it.key == INTERCEPT_CLOUDFLARE_KEY }
	...
	companion object {
		...
		private const val INTERCEPT_CLOUDFLARE_KEY = "intercept_cloudflare"
	}
}
