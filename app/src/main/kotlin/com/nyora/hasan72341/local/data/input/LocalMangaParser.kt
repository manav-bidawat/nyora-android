package com.nyora.hasan72341.local.data.input
...
import com.nyora.hasan72341.core.util.ext.isDirectory
import com.nyora.hasan72341.core.util.ext.isFileUri
import com.nyora.hasan72341.core.util.ext.isImage
import com.nyora.hasan72341.core.util.ext.isRegularFile
import com.nyora.hasan72341.core.util.ext.isZipUri
import com.nyora.hasan72341.core.util.ext.printStackTraceDebug
import com.nyora.hasan72341.core.util.ext.toFileNameSafe
import com.nyora.hasan72341.core.util.ext.toListSorted
import com.nyora.hasan72341.local.data.MangaIndex
import com.nyora.hasan72341.local.data.hasZipExtension
import com.nyora.hasan72341.local.data.isZipArchive
import com.nyora.hasan72341.local.data.output.LocalMangaOutput.Companion.ENTRY_NAME_INDEX
	...
	suspend fun getManga(withDetails: Boolean): LocalManga = runInterruptible(Dispatchers.IO) {
		(uri.resolveFsAndPath()).use { (fileSystem, rootPath) ->
			...
			if (mangaInfo != null) {
				...
				Manga(
					id = rootFile.absolutePath.longHashCode().toString(),
					title = title,
					url = rootFile.toUri().toString(),
					publicUrl = rootFile.toUri().toString(),
					source = MangaSourceRef.Local,
					coverUrl = fileSystem.findFirstImageUri(rootPath)?.toString().orEmpty(),
					chapters = if (withDetails) {
		...
		chapterUri.resolveFsAndPath().use { (fileSystem, rootPath) ->
			...
			if (index != null) {
				...
				.map { x ->
					...
					MangaPage(
						id = entryUri.longHashCode().toString(),
						url = entryUri,
						preview = null,
